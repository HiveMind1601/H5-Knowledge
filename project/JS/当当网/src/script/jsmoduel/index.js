define(['jquery','jquery.color','jquery.cookie','jquery.lazyload'],function(){
	return {
		getcookie:!function(){
			if ($.cookie('username')) {
		...
		wphdata:!function(){
			...
			.done(function(d){
				...
				$.each($qldata,function(index,value){
					if (index==0) {
						$qlstr+=`<a href="#"><img src=${$qldata[index].url}></a>`;
					}else{
						$qlstr+=`<ul>`;
						$qlstr+=`
								<li>
									<a href="#">
										<img src=${$qldata[index].url}>
									</a>
								</li>
						`;
						$qlstr+=`</ul>`;
				...
				$.each($qrdata,function(index,value){
					$qrstr+=`
							<li sid=${$qrdata[index].sid}>
								<a href="#"><img src=${$qrdata[index].url}></a>
							</li>
					`;
				});
				$qrstr+=`</ul>`;
